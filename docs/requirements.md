...
* **2026-03-21** — Implemented modal file previews: `preview_kind` on listing rows; `<dialog>` for image (prev/next in table), PDF (`iframe`), text (`/lister/preview.php` with Parsedown for `.md` / escaped plain for `.txt` / `<pre>` for other text kinds). `display.preview_text_max_bytes` in config. *Rationale:* Deliver section 2 and plan “browsing & previews” without new client libraries.
